...
export default async function handler(req, res) {
  try {
    ...
    const device = await pool.query("SELECT id FROM devices WHERE id=$1 AND status <> 'retired'", [Number(x.device_id)]);
    if (!device.rows[0]) return res.status(404).json({ error: 'ไม่พบเครื่อง Chromebook ที่เลือก' });
    ...
    console.error('[PUBLIC REPAIRS]', e);

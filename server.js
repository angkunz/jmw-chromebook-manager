...
async function init(){
  ...
  initialized = (async()=>{
    ...
    if(!password) throw new Error('ADMIN_PASSWORD is not configured in Vercel');
    const existing=await one('SELECT id FROM admin_users WHERE username=$1',[username]);
    if(!existing) await q('INSERT INTO admin_users(username,password_hash,role,active) VALUES($1,$2,$3,TRUE)',[username,makeHash(password),'admin']);
    return true;
  })();
  return initialized;
}
...
async function session(req){
  ...
  return one('SELECT s.*,a.username,a.role FROM sessions s JOIN admin_users a ON a.id=s.admin_id WHERE s.token_hash=$1 AND s.expires_at>NOW() AND a.active=TRUE',[sha(token)]);
}
...
app.put('/api/users/:id',auth,async(req,res,next)=>{try{const x=req.body||{};const old=await one('SELECT * FROM users WHERE id=$1',[req.params.id]);const r=await q('UPDATE users SET full_name=$1,type=$2,class_or_department=$3,email=$4,phone=$5,device_id=$6,borrow_date=$7,return_due_date=$8,return_reason=$9,active=$10,updated_at=NOW() WHERE id=$11 RETURNING *',[x.full_name,x.type||'student',x.class_or_department||null,x.email||null,x.phone||null,x.device_id||null,x.borrow_date||null,x.return_due_date||null,x.return_reason||null,x.active!==false,req.params.id]);if(old?.device_id&&old.device_id!==x.device_id)await q("UPDATE devices SET status='available',updated_at=NOW() WHERE id=$1 AND status='assigned'",[old.device_id]);if(x.device_id&&x.active!==false)await q("UPDATE devices SET status='assigned',updated_at=NOW() WHERE id=$1 AND status NOT IN ('repair','retired')",[x.device_id]);res.json(r.rows[0])}catch(e){res.status(400).json({error:e.message})}});
app.delete('/api/users/:id',auth,async(req,res,next)=>{try{const u=await one('SELECT device_id FROM users WHERE id=$1',[req.params.id]);await q('DELETE FROM users WHERE id=$1',[req.params.id]);if(u?.device_id)await q("UPDATE devices SET status='available' WHERE id=$1 AND status='assigned'",[u.device_id]);res.json({ok:true})}catch(e){next(e)}});
...
app.put('/api/repairs/:id',auth,async(req,res,next)=>{try{const x=req.body||{};const old=await one('SELECT * FROM repairs WHERE id=$1',[req.params.id]);const done=['completed','cancelled'].includes(x.status);const r=await q('UPDATE repairs SET status=$1,technician=$2,details=$3,updated_at=NOW(),completed_at=$4 WHERE id=$5 RETURNING *',[x.status,x.technician||null,x.details||null,done?new Date():null,req.params.id]);if(old)await q("UPDATE devices SET status=$1,updated_at=NOW() WHERE id=$2",[x.status==='completed'?'available':'repair',old.device_id]);res.json(r.rows[0])}catch(e){res.status(400).json({error:e.message})}});
...
app.use((err,req,res,next)=>{console.error('[JMW]',err);res.status(500).json({error:process.env.NODE_ENV==='production'?'เกิดข้อผิดพลาดภายในระบบ':err.message})});

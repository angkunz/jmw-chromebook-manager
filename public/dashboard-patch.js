(()=>{
  ...
  async function refresh(){
    ...
    // One active user can be linked to one device only. Count those assignments directly.
    const assignedCount=assignedUsers.length;
    // The latest non-completed/non-cancelled repair for a device means that device is currently under repair,
    // even when the device.status field has not yet caught up.
    const latestRepair=new Map();
    rs.forEach(r=>{const did=Number(r.device_id);if(!did)return;const prev=latestRepair.get(did);if(!prev||Number(r.id)>Number(prev.id))latestRepair.set(did,r)});
    const repairIds=new Set([...latestRepair.entries()].filter(([,r])=>!['completed','cancelled'].includes(r.status)).map(([did])=>did));
    const repairCount=repairIds.size;
    const retiredCount=ds.filter(d=>d.status==='retired').length;
    const availableCount=Math.max(0,ds.length-assignedCount-repairCount-retiredCount);
